...
def create_sounds_tab(app):
    ...
    enable_sound_cb = tk.Checkbutton(sounds_widget, text="Enable Sound?", font=("Arial", 11), variable=app.enable_sound)
    ...
    pips_dropdown = ttk.Combobox(sounds_widget, textvariable=app.pips_var, values=pips_options, state="readonly")
        ...
        if hasattr(self, '_user_interacting_with_pips') and app._user_interacting_with_pips:
            ...
            if selected != "Default" and selected != "No sound files found":
                if not check_audio_device_available(app.enable_sound):
                    app.audio_device_warning_shown = handle_no_audio_device_warning(
                        app.pips_var, "pips", app.enable_sound, app.audio_device_warning_shown)
            # Reset interaction flag
            app._user_interacting_with_pips = False
    
    app.pips_var.trace_add("write", validate_pips_selection)
    
    # Add event binding to detect user interaction
    def on_pips_user_interaction(event=None):
        app._user_interacting_with_pips = True
    ...
    def test_pips_sound():
        try:
            ...
            sound_file = app.pips_var.get()
            pips_vol = app.pips_volume.get()
            air_vol = app.air_volume.get()
            water_vol = app.water_volume.get()
            sound_enabled = app.enable_sound.get()
            ...
            print(log_msg)
            app.add_to_zigbee_log(f"Pips test: {sound_file} (Vol: {pips_vol}%, Air: {air_vol}%, Water: {water_vol}%)")
            
            # Check audio device availability
            audio_available = check_audio_device_available(app.enable_sound)
            device_msg = f"[{timestamp}] Audio device available: {audio_available}"
            print(device_msg)
            app.add_to_zigbee_log(f"Audio device available: {audio_available}")
            
            # Play the sound
            play_sound_with_volume(app.pips_var.get(), "pips", 
                app.enable_sound, app.pips_volume, app.siren_volume, 
                app.air_volume, app.water_volume, app.siren_duration)
            ...
            print(success_msg)
            app.add_to_zigbee_log("Pips sound playback initiated successfully")
            ...
            print(error_msg)
            app.add_to_zigbee_log(f"ERROR testing pips: {type(e).__name__}: {e}")
    ...
    pips_vol_slider = tk.Scale(
        sounds_widget, from_=0, to=100, orient="horizontal", variable=app.pips_volume,
    ...
    pips_vol_label = tk.Label(sounds_widget, text=f"{app.pips_volume.get()}%", 
    ...
    def on_pips_slider_interaction(event=None):
        ...
        if not check_audio_device_available(app.enable_sound):
            app.audio_device_warning_shown = handle_no_audio_device_warning(
                app.pips_var, "pips volume", app.enable_sound, app.audio_device_warning_shown)
    ...
    siren_dropdown = ttk.Combobox(sounds_widget, textvariable=app.siren_var, values=siren_options, state="readonly")
    ...
    def validate_siren_selection(*args):
        ...
        if hasattr(self, '_user_interacting_with_siren') and app._user_interacting_with_siren:
            ...
            if selected != "Default" and selected != "No sound files found":
                if not check_audio_device_available(app.enable_sound):
                    app.audio_device_warning_shown = handle_no_audio_device_warning(
                        app.siren_var, "siren", app.enable_sound, app.audio_device_warning_shown)
            # Reset interaction flag
            app._user_interacting_with_siren = False
    
    app.siren_var.trace_add("write", validate_siren_selection)
    
    # Add event binding to detect user interaction
    def on_siren_user_interaction(event=None):
        app._user_interacting_with_siren = True
    ...
    def test_siren_sound():
        try:
            # Get current settings
            sound_file = app.siren_var.get()
            siren_vol = app.siren_volume.get()
            air_vol = app.air_volume.get()
            water_vol = app.water_volume.get()
            sound_enabled = app.enable_sound.get()
            ...
            print(log_msg)
            app.add_to_zigbee_log(f"Siren test: {sound_file} (Vol: {siren_vol}%, Air: {air_vol}%, Water: {water_vol}%)")
            ...
            # Play the sound
            play_sound_with_volume(app.siren_var.get(), "siren",
                app.enable_sound, app.pips_volume, app.siren_volume,
                app.air_volume, app.water_volume, app.siren_duration)
            ...
            print(success_msg)
            app.add_to_zigbee_log("Siren sound playback initiated successfully")
            
        except Exception as e:
            error_timestamp = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
            error_msg = f"[{error_timestamp}] Error testing siren sound: {type(e).__name__}: {e}"
            print(error_msg)
            app.add_to_zigbee_log(f"ERROR testing siren: {type(e).__name__}: {e}")
    ...
    siren_vol_slider = tk.Scale(
        sounds_widget, from_=0, to=100, orient="horizontal", variable=app.siren_volume,
        font=("Arial", 10), showvalue=False
    )
    siren_vol_slider.grid(row=6, column=1, columnspan=2, sticky="ew")
    
    # Row 6, column 3: Siren volume value label
    siren_vol_label = tk.Label(sounds_widget, text=f"{app.siren_volume.get()}%", 
                               font=("Arial", 11), width=5)
    ...
    def on_siren_slider_interaction(event=None):
        ...
        if not check_audio_device_available(app.enable_sound):
            app.audio_device_warning_shown = handle_no_audio_device_warning(
                app.siren_var, "siren volume", app.enable_sound, app.audio_device_warning_shown)
    ...
    siren_duration_entry = tk.Entry(sounds_widget, textvariable=app.siren_duration, font=("Arial", 11), width=10)
    ...
    def normalize_siren_duration(event=None):
        try:
            current_value = app.siren_duration.get()
    ...
    air_vol_slider = tk.Scale(
        sounds_widget, from_=100, to=0, orient="vertical", variable=app.air_volume,
        font=("Arial", 10), showvalue=False
    )
    air_vol_slider.grid(row=2, column=4, rowspan=5, sticky="ns")
    
    # Row 8, column 4: Air volume value label
    air_vol_label = tk.Label(sounds_widget, text=f"{app.air_volume.get()}%", 
                             font=("Arial", 11))
    ...
    def on_air_slider_interaction(event=None):
        ...
        if not check_audio_device_available(app.enable_sound):
            app.audio_device_warning_shown = handle_no_audio_device_warning(
                tk.StringVar(value="Default"), "air volume", app.enable_sound, app.audio_device_warning_shown)
    ...
    water_vol_slider = tk.Scale(
        sounds_widget, from_=100, to=0, orient="vertical", variable=app.water_volume,
        font=("Arial", 10), showvalue=False
    )
    water_vol_slider.grid(row=2, column=5, rowspan=5, sticky="ns")
    
    # Row 8, column 5: Water volume value label
    water_vol_label = tk.Label(sounds_widget, text=f"{app.water_volume.get()}%", 
                               font=("Arial", 11))
    ...
    def on_water_slider_interaction(event=None):
        ...
        if not check_audio_device_available(app.enable_sound):
            app.audio_device_warning_shown = handle_no_audio_device_warning(
                tk.StringVar(value="Default"), "water volume", app.enable_sound, app.audio_device_warning_shown)
